...
def test_platform_promotion_artifacts_reject_manifest_architecture_drift(tmp_path: Path) -> None:
    ...
    assert any(
        ...
        "remote-ops-workspace-v1.0.11-linux-armhf.deb architecture must be 'armhf', got 'arm64'"
        in error
        for error in errors
    )
...
def test_platform_promotion_artifacts_reject_manifest_format_drift(tmp_path: Path) -> None:
    ...
    assert any(
        ...
        "remote-ops-workspace-v1.0.11-windows-xp-x64-native.zip format must be 'zip', got 'tar.gz'"
        in error
        for error in errors
    )
