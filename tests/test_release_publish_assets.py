...
def test_expected_release_assets_expand_default_matrix() -> None:
    ...
    assert "remote_ops_workspace-1.0.0-py3-none-any.whl" in assets
    assert "remote-ops-workspace-v1.0.0-windows-x86-setup.exe" in assets
    assert "remote-ops-workspace-v1.0.0-macos-arm64.pkg" in assets
    assert "remote-ops-workspace-v1.0.0-linux-amd64.deb" in assets
    assert "remote-ops-workspace-v1.0.0-linux-aarch64-native-SHA256SUMS.txt" in assets
...
def test_release_assets_report_missing_expected_files(tmp_path: Path) -> None:
    ...
    errors = checker.check_release_assets(tmp_path, matrix, tag="v1.0.0")
...
def test_release_assets_reject_checksum_mismatch(tmp_path: Path) -> None:
    ...
    matrix = _load_matrix()
    _write_synthetic_release_assets(checker, matrix, tmp_path)
    checksum = tmp_path / "remote-ops-workspace-v1.0.0-SHA256SUMS.txt"
    checksum.write_text("0" * 64 + "  remote_ops_workspace-1.0.0-py3-none-any.whl\n", encoding="utf-8")
...
def _write_synthetic_release_assets(checker, matrix: dict[str, object], root: Path) -> None:
    expected = checker.expected_release_assets(matrix, tag="v1.0.0")
    source_manifest_artifacts = checker.expected_source_manifest_artifacts(matrix, tag="v1.0.0")
    release_manifest = "remote-ops-workspace-v1.0.0-release-manifest.json"
