...
def create_main_window(argv: list[str] | None = None, *, show: bool = False):
    ...
    class MobaSftpDock(QFrame):
        ...
        def __init__(self, state: MobaConnectedSessionState) -> None:
            ...
            self.monitoring_process.readyReadStandardOutput.connect(
                self.read_remote_monitoring_output
            )
            self.monitoring_process.finished.connect(
                self.handle_remote_monitoring_finished
            )
            self.monitoring_process.errorOccurred.connect(
                self.handle_remote_monitoring_error
            )
            self.sftp_refresh_output_buffer = bytearray()
            self.sftp_refresh_plan: SftpBatchPlan | None = None
            self.sftp_refresh_generation = 0
            ...
            self.sftp_refresh_process.started.connect(
                self.write_sftp_refresh_batch
            )
            self.sftp_refresh_process.readyReadStandardOutput.connect(
                self.read_sftp_refresh_output
            )
            self.sftp_refresh_process.finished.connect(
                self.handle_sftp_refresh_finished
            )
            self.sftp_refresh_process.errorOccurred.connect(
                self.handle_sftp_refresh_error
            )
            self.sftp_refresh_timeout = QTimer(self)
            ...
            path.returnPressed.connect(self.handle_moba_sftp_path_entered)
            self.path = path
            ...
            file_header = _required_gui_value(
                self.file_table.header(),
                "SFTP file table header",
            )
            file_header.setFixedHeight(density.table_header_height)
            file_header.setSectionResizeMode(QHeaderView.ResizeMode.Fixed)
            ...
            self.file_table.currentItemChanged.connect(self.update_sftp_action_states)
            self.file_table.itemDoubleClicked.connect(self.handle_moba_text_editor_open_from_item)
            self.file_table.customContextMenuRequested.connect(self.show_sftp_context_menu)
        ...
        def set_sftp_runtime_status(self, message: str, *, state: str) -> None:
            ...
            if badge is not None:
                ...
                badge.setIcon(
                    _widget_style(self).standardIcon(status_icon)
                    if status_icon is not None
                    else self._sftp_status_default_icon
                )
                badge_style = _widget_style(badge)
        ...
        def text_editor_action_button(self, action_key: str, icon_name: str, object_name: str, handler) -> QToolButton:
            ...
            button.setIcon(_widget_style(self).standardIcon(self.standard_icon(icon_name)))
        ...
        def text_editor_remote_path_for_item(self, item: QTreeWidgetItem) -> str:
            ...
            if kind != "file" or not name:
                return ""
            base = str(
                item.data(0, SFTP_ROW_SOURCE_PATH_ROLE)
                or self.active_remote_path
            ).rstrip("/")
            if not base:
                base = "/"
        ...
        def apply_sftp_routed_file_row_metadata(
            self,
            ...
            *,
            ...
            routed_path = normalise_remote_path(
                source_path or self.active_remote_path
            )
            item.setData(0, SFTP_ROW_CONTRACT_KEY_ROLE, rows.key)
            item.setData(0, SFTP_ROW_ROUTE_KEY_ROLE, rows.follow_route_key)
            item.setData(0, SFTP_ROW_SOURCE_PATH_ROLE, routed_path)
            ...
            item.setToolTip(
                1,
                _safe_tooltip_html(f"{rows.row_path_property}: {routed_path}"),
            )
        ...
        def build_remote_monitoring(self, density) -> QFrame:
            ...
            self.monitoring_status_label = status
            refresh_button = QToolButton(controls)
            refresh_button.setObjectName("mobaMonitoringRefresh")
            refresh_button.setText("Refresh now")
            refresh_button.setToolButtonStyle(
            ...
            remote_control = self.monitoring_control_widgets.get("remote-monitoring")
            self.set_remote_monitoring_expanded(
                bool(remote_control is not None and remote_control.isChecked())
            )
            return panel
        ...
        def monitoring_control_widget(self, control):
            ...
            control_font.setHintingPreference(
        ...
        def activate_initial_monitoring_state(self) -> None:
            control = self.monitoring_control_widgets.get("remote-monitoring")
        ...
        def set_remote_monitoring_expanded(self, expanded: bool) -> None:
        ...
        def set_remote_monitoring_status(
            ...
            *,
            ...
            label = getattr(self, "monitoring_status_label", None)
            if label is not None:
                label.setText(message)
                label.setProperty("state", state)
                label.setToolTip(_safe_tooltip_html(message))
                label_style = _widget_style(label)
                label_style.unpolish(label)
                label_style.polish(label)
                label.update()
            control = self.monitoring_control_widgets.get("remote-monitoring")
            if control is not None:
                ...
                control_style = _widget_style(control)
                control_style.unpolish(control)
                control_style.polish(control)
                control.update()
            panel = getattr(self, "remote_monitoring_panel", None)
            for widget in (self, panel):
                if widget is not None:
                    widget.setProperty("mobaRemoteMonitoringRuntimeState", state)
                    widget.setProperty("mobaRemoteMonitoringRuntimeMessage", message)

        def monitoring_surface_is_current(self) -> bool:
            if self.runtime_shutting_down:
                return False
            main_window = self.main_window()
            if main_window is not None and hasattr(main_window, "moba_connected_dock"):
                return main_window.moba_connected_dock is self
            return True

        def remote_monitoring_request_is_current(self) -> bool:
            control = self.monitoring_control_widgets.get("remote-monitoring")
            return (
                self.monitoring_surface_is_current()
                and self.monitoring_active_generation != 0
        ...
        def can_reuse_for_state(
            self,
            state: MobaConnectedSessionState,
        ) -> bool:
            return (
                not self.runtime_shutting_down
                and self.state.profile_name == state.profile_name
                and self.state.target == state.target
                and self.active_remote_path == state.remote_path
                and self.state.monitoring_plan == state.monitoring_plan
                and self.state.sftp_list_plan == state.sftp_list_plan
                and self.state.follow_folder_plan == state.follow_folder_plan
                and self.state.banner == state.banner
                and self.state.smartcard_selection
                == state.smartcard_selection
            )

        def set_remote_monitoring_runtime(
            self,
            enabled: bool,
            *,
            immediate: bool,
            ...
            for widget in (
                ...
                getattr(self, "remote_monitoring_controls", None),
            ):
                if widget is not None:
                    widget.setProperty(
                        "mobaRemoteMonitoringRuntimeActive",
                        active,
                    )
            ...
            if active:
                ...
                if immediate:
                    QTimer.singleShot(0, self.request_remote_monitoring_refresh)
                return
            self.monitoring_generation += 1
            self.monitoring_active_generation = 0
            self.monitoring_refresh_timer.stop()
            ...
            self.set_remote_monitoring_status("Monitoring off", state="paused")

        def monitoring_runtime_command(self) -> list[str]:
        ...
        def request_remote_monitoring_refresh(self, *_args) -> None:
            ...
            for widget in (
                ...
                if widget is not None:
                    widget.setProperty(
                        "mobaRemoteMonitoringRefreshRequestCount",
                        count,
                    )
            if self.monitoring_process.state() != QProcess.ProcessState.NotRunning:
                self.set_remote_monitoring_status(
                    "Monitoring refresh already running",
                    state="refreshing",
                )
                return
            command = self.monitoring_runtime_command()
            if not command:
                self.set_remote_monitoring_status(
                    "Monitoring unavailable: empty command",
                    state="error",
                )
                return
            ...
            self.monitoring_process.setProgram(command[0])
            self.monitoring_process.setArguments(command[1:])
        ...
        def apply_live_remote_monitoring_snapshot(self, snapshot) -> bool:
            ...
            for cell in moba_telemetry_cells(live_state):
                frame = next(
                    (
                        candidate
                        for candidate in telemetry_bar.findChildren(
                            QFrame,
                            "mobaTelemetryCell",
                        )
                        if candidate.property("mobaTelemetryKey") == cell.key
                    ),
                    None,
                )
                if frame is None:
                ...
                label = next(
                    (
                        candidate
                        for candidate in frame.findChildren(
                            QLabel,
                            "mobaTelemetryItem",
                        )
                        if candidate.property("mobaTelemetryKey") == cell.key
        ...
        def build_remote_monitoring_context_menu(self) -> QMenu:
            menu = QMenu(self)
            menu.setObjectName("mobaRemoteMonitoringContextMenu")
            control = self.monitoring_control_widgets.get("remote-monitoring")
            enabled_action = _required_gui_value(
                menu.addAction("Remote monitoring enabled"),
                "remote-monitoring action",
            )
            enabled_action.setCheckable(True)
            enabled_action.setChecked(
                bool(control is not None and control.isChecked())
            )
            if control is not None:
                enabled_action.toggled.connect(control.setChecked)
            ...
            follow_action = _required_gui_value(
                menu.addAction("Follow terminal folder"),
                "follow-terminal-folder action",
            )
            ...
            menu.addAction(
                ...
                lambda: _application_clipboard().setText(
                    self.state.monitoring_plan.printable()
                ),
            )
            return menu

        def show_remote_monitoring_context_menu(self, position: QPoint) -> None:
            menu = self.build_remote_monitoring_context_menu()
            menu.exec(self.remote_monitoring_panel.mapToGlobal(position))
            menu.deleteLater()
        ...
        def handle_moba_remote_monitoring_toggled(self, checked: bool) -> None:
            ...
            self.set_remote_monitoring_runtime(bool(checked), immediate=bool(checked))
        ...
        def handle_moba_follow_terminal_folder_toggled(self, checked: bool) -> None:
            ...
            if checked:
                self.request_sftp_refresh(reason="follow-enabled")
            elif self.sftp_refresh_process.state() != QProcess.ProcessState.NotRunning:
                self.sftp_refresh_generation += 1
                self.sftp_refresh_active_generation = 0
                self.sftp_refresh_pending = None
        ...
        def request_sftp_refresh(self, *, reason: str = "manual") -> None:
            ...
            for widget in (
                ...
                if widget is not None:
                    widget.setProperty("mobaSftpRefreshRequestCount", count)
                    widget.setProperty("mobaSftpRefreshLastReason", reason)
            request_path = normalise_remote_path(self.active_remote_path)
            self.sftp_refresh_generation += 1
        ...
        def write_sftp_refresh_batch(self) -> None:
            ...
            if plan is None:
                return
            accepted = self.sftp_refresh_process.write(
                plan.batch_input().encode("utf-8")
            )
            self.sftp_refresh_process.closeWriteChannel()
            if accepted < 0:
                self.show_sftp_status(
                    "SFTP refresh failed: batch input was not accepted"
                )
        ...
        def sftp_refresh_request_is_current(self) -> bool:
            return (
                not self.runtime_shutting_down
                and self.sftp_refresh_active_generation != 0
                and self.sftp_refresh_active_generation
                == self.sftp_refresh_generation
                and self.sftp_refresh_request_path
                == normalise_remote_path(self.active_remote_path)
            )

        def schedule_pending_sftp_refresh(self) -> None:
            pending = self.sftp_refresh_pending
            self.sftp_refresh_pending = None
            if pending is None or self.runtime_shutting_down:
                return
            pending_path, pending_reason = pending

            def start_pending() -> None:
                if (
                    not self.runtime_shutting_down
                    and normalise_remote_path(self.active_remote_path)
                    == pending_path
                ):
                    self.request_sftp_refresh(reason=pending_reason)

            QTimer.singleShot(0, start_pending)

        def handle_sftp_refresh_finished(
            self,
            exit_code: int,
            _exit_status: QProcess.ExitStatus,
        ) -> None:
            ...
            self.sftp_refresh_active_generation = 0
            if not request_is_current:
                if not self.runtime_shutting_down:
                    self.show_sftp_status("SFTP refresh superseded")
                self.schedule_pending_sftp_refresh()
                return
            if exit_code != 0:
                detail = next(
        ...
        def apply_live_sftp_entries(
            ...
            *,
            ...
            follow_route = gui_design_moba_sftp_follow_folder_route()
            routed_rows = gui_design_moba_sftp_routed_file_rows()
            selected_path = normalise_remote_path(request_path)
            self.state = replace(
                self.state,
            ...
            for row_index, entry in enumerate(entries, start=1):
                item = QTreeWidgetItem(
                    [entry.name, str(entry.size_kb), entry.modified]
                )
                self.apply_sftp_file_row_icon(item, entry.kind)
                self.apply_sftp_routed_file_row_metadata(
                    item,
                ...
                item.setToolTip(
                    0,
                    _safe_tooltip_html(f"{entry.kind}: {entry.name}"),
                )
                self.file_table.addTopLevelItem(item)
            parent_item.setSelected(True)
            self.file_table.setCurrentItem(parent_item)
            self.apply_sftp_follow_folder_route_properties(
                self.file_table,
                follow_route,
            )
            self.update_sftp_action_states()
        ...
        def update_sftp_action_states(self, *_args) -> None:
            selected = self.file_table.currentItem() if hasattr(self, "file_table") else None
            selected_name = selected.text(0).strip() if selected is not None else ""
            selected_kind = (
        ...
        def navigate_moba_sftp_path(self, raw_path: str) -> bool:
            try:
                normalized = posixpath.normpath(normalise_remote_path(raw_path))
            except ValueError as exc:
                self.path.setText(self.active_remote_path)
                self.path.selectAll()
                self.path.setProperty("mobaSftpPathNavigationError", str(exc))
                self.show_sftp_status(f"SFTP path rejected: {exc}")
                return False
            if not normalized.startswith("/"):
                normalized = f"/{normalized}"
            self.active_remote_path = normalized
            self.path.setText(normalized)
            navigation_count = int(self.path.property("mobaSftpPathNavigationCount") or 0) + 1
            route_widgets = (
                self,
                getattr(self, "browser", None),
                self.path,
            ...
            for widget in route_widgets:
                if widget is None:
                    continue
                widget.setProperty("mobaSftpLivePath", normalized)
                widget.setProperty("mobaSftpPathNavigationCount", navigation_count)
                widget.setProperty("mobaSftpPathNavigationError", "")
                widget.setProperty("mobaSftpFollowRoutePath", normalized)
                widget.setProperty("mobaSftpTerminalFolderRoutePath", normalized)
                widget.setProperty("mobaConnectedRouteRemotePath", normalized)
            if hasattr(self, "file_table"):
                for index in range(self.file_table.topLevelItemCount()):
                    item = self.file_table.topLevelItem(index)
                    if item is not None:
                        item.setData(0, SFTP_ROW_SOURCE_PATH_ROLE, normalized)
            if hasattr(self, "sftp_transfer_queue"):
                self.sftp_transfer_queue.setText(f"Path: {normalized}")
            self.show_sftp_status(f"SFTP path: {normalized}")
            follow_control = self.monitoring_control_widgets.get(
                "follow-terminal-folder"
            )
            if follow_control is not None and follow_control.isChecked():
                self.request_sftp_refresh(reason="path-change")
            return True

        def selected_sftp_item(self) -> tuple[str, str] | None:
            item = self.file_table.currentItem() if hasattr(self, "file_table") else None
            if item is None:
                return None
            name = item.text(0).strip()
            kind = str(item.data(0, SFTP_ROW_KIND_ROLE) or "")
            if not name or name == ".." or kind not in {"file", "dir"}:
                return None
            return name, kind

        def sftp_remote_path_for_item(self, item: QTreeWidgetItem | None) -> str:
            if item is None:
                return self.active_remote_path
            name = item.text(0).strip()
            kind = str(item.data(0, SFTP_ROW_KIND_ROLE) or "")
            if name == ".." or kind == "parent-dir":
                return str(PurePosixPath(self.active_remote_path).parent)
            return str(PurePosixPath(self.active_remote_path) / name)
        ...
        def open_sftp_context_item(self, item: QTreeWidgetItem | None) -> None:
            ...
            name = item.text(0).strip()
            kind = str(item.data(0, SFTP_ROW_KIND_ROLE) or "")
            if name == ".." or kind in {"dir", "parent-dir"}:
                self.navigate_moba_sftp_path(self.sftp_remote_path_for_item(item))
                return
            self.handle_moba_text_editor_open_from_item(item, 0)

        def copy_sftp_context_path(self, item: QTreeWidgetItem | None) -> None:
            path = self.sftp_remote_path_for_item(item)
            _application_clipboard().setText(path)
            self.show_sftp_status(f"Copied remote path: {path}")

        def build_sftp_context_menu(
            self,
            ...
            selected = item or self.file_table.currentItem()
            kind = (
                str(selected.data(0, SFTP_ROW_KIND_ROLE) or "")
                if selected is not None
                else ""
            )
            name = selected.text(0).strip() if selected is not None else ""
            open_ready = bool(selected is not None and name)
            transfer_ready = kind in {"file", "dir"} and name not in {"", ".."}
            menu = QMenu(self.file_table)
            menu.setObjectName("mobaSftpContextMenu")
            open_action = _required_gui_value(
                menu.addAction(
                    "Open" if kind == "file" else "Open folder",
                    lambda: self.open_sftp_context_item(selected),
                ),
                "SFTP open action",
            )
            open_action.setEnabled(open_ready)
            menu.addSeparator()
            download_action = _required_gui_value(
                menu.addAction(
                    "Download",
                    lambda: self.show_moba_sftp_toolbar_action("download"),
                ),
                "SFTP download action",
            )
            download_action.setEnabled(transfer_ready)
            menu.addAction(
                "Upload here",
                lambda: self.show_moba_sftp_toolbar_action("upload"),
            )
            copy_action = _required_gui_value(
                menu.addAction(
                    "Copy remote path",
                    lambda: self.copy_sftp_context_path(selected),
                ),
                "SFTP copy-path action",
            )
            copy_action.setEnabled(open_ready)
            menu.addSeparator()
            menu.addAction(
                "Parent folder",
            ...
            menu.addAction(
                "Refresh listing",
                lambda: self.request_sftp_refresh(reason="context-menu"),
            )
            menu.addAction(
                "Reconnect",
                lambda: self.show_moba_sftp_toolbar_action("connect"),
            )
            menu.addAction(
                "Focus terminal",
                lambda: self.show_moba_sftp_toolbar_action("terminal"),
            )
            return menu

        def show_sftp_context_menu(self, position: QPoint) -> None:
            item = self.file_table.itemAt(position)
            if item is not None:
                self.file_table.setCurrentItem(item)
            menu = self.build_sftp_context_menu(item)
            viewport = _required_gui_value(
                self.file_table.viewport(),
                "SFTP file table viewport",
            )
            menu.exec(viewport.mapToGlobal(position))
            menu.deleteLater()

        def profile_for_sftp_action(self) -> Profile | None:
            main_window = self.main_window()
            if main_window is None:
                return None
            return main_window.profile_by_name(self.state.profile_name)

        def show_sftp_status(self, message: str) -> None:
            main_window = self.main_window()
            if main_window is None:
                return
            ...
            main_window.log.append(message)

        def open_moba_sftp_transfer_workflow(self, action_key: str) -> bool:
            main_window = self.main_window()
            profile = self.profile_for_sftp_action()
            if profile is None or main_window is None:
                self.show_sftp_status("SFTP transfer unavailable: connected profile was not found")
                return False
            if action_key == "download":
                selected = self.selected_sftp_item()
                if selected is None:
                    self.show_sftp_status("Select a remote file or directory to download")
                    return False
                name, kind = selected
                remote_path = str(PurePosixPath(self.active_remote_path) / name)
                ...
                local_path, _selected_filter = QFileDialog.getOpenFileName(
                    self,
                ...
                if not local_path:
                    self.show_sftp_status("SFTP upload cancelled")
                    return False
                remote_path = str(PurePosixPath(self.active_remote_path) / Path(local_path).name)
                operation = shlex.join(["put", local_path, remote_path])
        ...
        def reconnect_moba_sftp_session(self) -> bool:
            main_window = self.main_window()
            if main_window is None:
                self.show_sftp_status("Reconnect unavailable: main window is unavailable")
                return False
            current = main_window.tabs.currentWidget()
            panes = main_window.terminal_panes_in(current) if current is not None else []
            pane = next(
                (
                    item
                    for item in panes
                    if item.profile is not None and item.profile.name == self.state.profile_name
                ),
                panes[0] if panes else None,
            )
            if pane is None:
                self.show_sftp_status("Reconnect unavailable: no terminal pane is attached")
                return False
            pane.restart()
            terminal_output = pane.output
            terminal_output.setProperty("mobaTerminalFocusRequested", True)
            terminal_output.setFocus(Qt.FocusReason.OtherFocusReason)
            QTimer.singleShot(
                0,
                lambda: terminal_output.setFocus(Qt.FocusReason.OtherFocusReason),
            )
            self.show_sftp_status(f"Reconnecting {self.state.profile_name}")
            return True

        def focus_moba_sftp_terminal(self) -> bool:
            main_window = self.main_window()
            pane = main_window.active_terminal_pane() if main_window is not None else None
            if pane is None:
                self.show_sftp_status("Terminal unavailable: no active session pane")
                return False
            terminal_output = pane.output
            terminal_output.setProperty("mobaTerminalFocusRequested", True)
            terminal_output.setFocus(Qt.FocusReason.OtherFocusReason)
            ...
            self.show_sftp_status(f"Terminal focused at {self.active_remote_path}")
            return True

        def dispatch_moba_sftp_toolbar_action(self, action_key: str) -> bool:
            if bool(self.property("mobaSftpToolbarRouteSuppressDialog")):
                return True
            if action_key == "parent-folder":
                parent = str(PurePosixPath(self.active_remote_path).parent)
                return self.navigate_moba_sftp_path(parent)
            if action_key in {"download", "upload"}:
                return self.open_moba_sftp_transfer_workflow(action_key)
            if action_key == "connect":
                return self.reconnect_moba_sftp_session()
            if action_key == "tools":
                main_window = self.main_window()
                if main_window is None:
                    return False
                main_window.show_moba_tools_status()
                return True
            if action_key == "terminal":
                return self.focus_moba_sftp_terminal()
            return False
        ...
        def show_moba_sftp_toolbar_action(self, action_key: str) -> None:
            ...
            operational = action.key in self.OPERATIONAL_ACTIONS
            status_value = route.action_statuses[action_index] if operational else "unavailable"
            ...
            if not operational:
                self.show_sftp_status(
                    f"SFTP {action.label} unavailable: "
                    f"{self.UNAVAILABLE_ACTION_DETAILS.get(action.key, action.tooltip)}"
                )
                return
        ...
        def tool_button(self, action, density) -> QToolButton:
            ...
            button.setToolTip(
                action.tooltip
                if operational
                else self.UNAVAILABLE_ACTION_DETAILS.get(action.key, action.tooltip)
            )
    ...
    class MobaConnectedSessionPanel(QWidget):
        ...
        def build_moba_terminal_context_menu(self, pane: TerminalPane) -> QMenu:
            ...
            copy_action = _required_gui_value(
                menu.addAction("Copy"),
                "terminal context copy action",
            )
            copy_action.setEnabled(selection)
            copy_action.triggered.connect(pane.copy_terminal_selection)
            save_action = _required_gui_value(
                menu.addAction("Save to file"),
                "terminal context save action",
            )
            ...
            for key, frame in getattr(self, "telemetry_cell_frames", {}).items():
                ...
                action = _required_gui_value(
                    menu.addAction(f"Display {label} information"),
                    f"terminal telemetry action for {key}",
                )
            ...
            paste_action = _required_gui_value(
                ...
                "terminal context paste action",
            )
            paste_action.setEnabled(bool(_application_clipboard().text()))
            paste_action.triggered.connect(pane.paste_to_terminal)
            select_action = _required_gui_value(
                menu.addAction("Select all"),
                "terminal context select-all action",
            )
            select_action.triggered.connect(pane.output.selectAll)

            dock = self.active_moba_sftp_dock()
            if dock is not None:
                monitoring_menu = _required_gui_value(
                    menu.addMenu("Monitoring"),
                    "terminal monitoring menu",
                )
                monitoring_menu.setObjectName("mobaMonitoringContextMenu")
                monitoring_control = dock.monitoring_control_widgets.get(
                    "remote-monitoring"
                )
                monitoring_action = _required_gui_value(
                    monitoring_menu.addAction("Remote monitoring enabled"),
                    "remote monitoring toggle action",
                )
                ...
                follow_action = _required_gui_value(
                    monitoring_menu.addAction("Follow terminal folder"),
                    "follow terminal folder action",
                )
                follow_action.setCheckable(True)
                follow_action.setChecked(
            ...
            tools_menu = _required_gui_value(
                menu.addMenu("Session tools"),
                "terminal session tools menu",
            )
            ...
            for session_action_spec in gui_design_moba_session_edge_actions():
                action = _required_gui_value(
                    tools_menu.addAction(session_action_spec.label),
                    f"session edge action for {session_action_spec.key}",
                )
                action.setProperty("mobaSessionEdgeKey", session_action_spec.key)
                action.triggered.connect(
                    lambda _checked=False, handler=session_handlers[
                        session_action_spec.key
            ...
            for utility_action_spec in gui_design_moba_right_utility_actions():
                action = _required_gui_value(
                    tools_menu.addAction(utility_action_spec.label),
                    f"right utility action for {utility_action_spec.key}",
                )
                action.setProperty("mobaRightUtilityKey", utility_action_spec.key)
                action.triggered.connect(
                    lambda _checked=False, handler=utility_handlers[
                        utility_action_spec.key
                    ]: self.dispatch_moba_workspace_action(handler)
                )
        ...
        def apply_ssh_banner_font(
            ...
            *,
            ...
            label.setFont(font)
            label.setProperty("mobaBannerTextHinting", "PreferFullHinting")
            label.setProperty("mobaBannerTextPixelSize", pixel_size)
        ...
        def build_ssh_banner(self) -> QFrame:
            ...
            self.apply_ssh_banner_font(title, pixel_size=14, bold=True)
            ...
            self.apply_ssh_banner_font(subtitle, pixel_size=12)
            ...
            self.apply_ssh_banner_font(target, pixel_size=12)
            ...
            for row in self.state.banner.capability_rows():
                ...
                self.apply_ssh_banner_font(label, pixel_size=12)
            ...
            self.apply_ssh_banner_font(footer, pixel_size=12)
    ...
    class MainWindow(QMainWindow):
        ...
        def __init__(self) -> None:
            ...
            self.toolbar_widget_actions: dict[int, QAction] = {}
            ...
            self.moba_tab_bar.setContextMenuPolicy(
            ...
            self.moba_tab_bar.customContextMenuRequested.connect(
        ...
        def create_keyboard_shortcuts(self) -> list:
            ...
            for spec in self.keyboard_shortcut_specs():
                ...
                if not callable(callback):
                    raise RuntimeError(
                        f"keyboard shortcut callback is unavailable: {spec['key']}"
                    )
        ...
        def build_menu_bar(self) -> None:
            self.main_menu_bar = _required_gui_value(
                self.menuBar(),
                "main menu bar",
            )
            self.main_menu_bar.setObjectName("mobaTopMenuBar")
            self.moba_top_menus: list[QMenu] = []
            self.moba_top_menu_actions: list[QAction] = []
            ...
            for moba_item in gui_design_moba_top_menu_items():
                geometry = gui_design_moba_top_menu_geometry_for(moba_item.key)
                menu = _required_gui_value(
                    self.main_menu_bar.addMenu(moba_item.label),
                    f"Moba top menu for {moba_item.key}",
                )
                menu.setObjectName("mobaTopMenu")
                menu.setProperty("mobaTopMenuKey", moba_item.key)
                menu.setProperty("mobaTopMenuLabel", moba_item.label)
                menu.setProperty(
                    "mobaTopMenuGeometryKeys",
                    [entry.key for entry in gui_design_moba_top_menu_geometry()],
                )
                ...
                menu.setToolTip(moba_item.tooltip)
                menu_action = _required_gui_value(
                    menu.menuAction(),
                    f"Moba menu action for {moba_item.key}",
                )
                menu_action.setProperty("mobaTopMenuKey", moba_item.key)
                menu_action.setProperty("mobaTopMenuLabel", moba_item.label)
                menu_action.setProperty(
                    "mobaTopMenuGeometryKeys",
                    [entry.key for entry in gui_design_moba_top_menu_geometry()],
                )
                menu_action.setProperty("mobaTopMenuStaticX", geometry.static_x)
                menu_action.setProperty("mobaTopMenuWidth", geometry.width)
                menu_action.setProperty("mobaTopMenuLabelY", geometry.label_y)
                menu_action.setProperty(
                    "mobaTopMenuLabelFontSize",
                    geometry.label_font_size,
                )
                menu_action.setProperty("mobaTopMenuGapAfter", geometry.gap_after)
                menu_action.setToolTip(moba_item.tooltip)
                self.moba_top_menus.append(menu)
                self.moba_top_menu_actions.append(menu_action)
                if moba_item.key == "sessions":
                    menu.addAction("New session", self.create_profile)
                if moba_item.key == "view":
                    self.view_menu = menu
                if moba_item.key not in self.product_menu_callbacks["mobaxterm"]:
                    raise RuntimeError(
                        f"missing Moba top-menu handler: {moba_item.key}"
                    )
                action = _required_gui_value(
                    menu.addAction(moba_item.primary_action),
                    f"Moba primary menu action for {moba_item.key}",
                )
                action.triggered.connect(
                    lambda _checked=False, action_key=moba_item.key: self.run_product_menu_action(
                        "mobaxterm", action_key
                    )
                )
                action.setProperty("menuActionKey", moba_item.key)
                action.setProperty("menuActionFamily", "mobaxterm")
                action.setProperty(
                    "menuActionOperation",
                    self.product_menu_operations["mobaxterm"][moba_item.key],
                )

            securecrt_chrome = gui_design_securecrt_top_chrome()
            self.securecrt_top_menus: list[QMenu] = []
            self.securecrt_top_menu_actions: list[QAction] = []
            for securecrt_item in securecrt_chrome.menu_items:
                menu = _required_gui_value(
                    self.main_menu_bar.addMenu(securecrt_item.label),
                    f"SecureCRT top menu for {securecrt_item.key}",
                )
                menu.setObjectName("secureCrtTopMenu")
                menu.setProperty("secureCrtTopMenuKey", securecrt_item.key)
                menu.setProperty("secureCrtTopMenuLabel", securecrt_item.label)
                menu.setProperty(
                    "secureCrtTopMenuPrimaryAction",
                    securecrt_item.primary_action,
                )
                menu.setToolTip(securecrt_item.tooltip)
                menu_action = _required_gui_value(
                    menu.menuAction(),
                    f"SecureCRT menu action for {securecrt_item.key}",
                )
                menu_action.setProperty("secureCrtTopMenuKey", securecrt_item.key)
                menu_action.setProperty("secureCrtTopMenuLabel", securecrt_item.label)
                menu_action.setProperty(
                    "secureCrtTopMenuPrimaryAction",
                    securecrt_item.primary_action,
                )
                menu_action.setToolTip(securecrt_item.tooltip)
                self.securecrt_top_menus.append(menu)
                self.securecrt_top_menu_actions.append(menu_action)
                if securecrt_item.key not in self.product_menu_callbacks["securecrt"]:
                    raise RuntimeError(
                        f"missing SecureCRT top-menu handler: {securecrt_item.key}"
                    )
                action = _required_gui_value(
                    menu.addAction(securecrt_item.primary_action),
                    f"SecureCRT primary menu action for {securecrt_item.key}",
                )
                action.triggered.connect(
                    lambda _checked=False, action_key=securecrt_item.key: self.run_product_menu_action(
                        "securecrt", action_key
                    )
                )
                action.setProperty("menuActionKey", securecrt_item.key)
                action.setProperty("menuActionFamily", "securecrt")
                action.setProperty(
                    "menuActionOperation",
                    self.product_menu_operations["securecrt"][securecrt_item.key],
                )

            mremoteng_chrome = gui_design_mremoteng_top_chrome()
            self.mremoteng_top_menus: list[QMenu] = []
            self.mremoteng_top_menu_actions: list[QAction] = []
            for mremoteng_item in mremoteng_chrome.menu_items:
                menu = _required_gui_value(
                    self.main_menu_bar.addMenu(mremoteng_item.label),
                    f"mRemoteNG top menu for {mremoteng_item.key}",
                )
                menu.setObjectName("mRemoteNgTopMenu")
                menu.setProperty("mRemoteNgTopMenuKey", mremoteng_item.key)
                menu.setProperty("mRemoteNgTopMenuLabel", mremoteng_item.label)
                menu.setProperty(
                    "mRemoteNgTopMenuPrimaryAction",
                    mremoteng_item.primary_action,
                )
                menu.setToolTip(mremoteng_item.tooltip)
                menu_action = _required_gui_value(
                    menu.menuAction(),
                    f"mRemoteNG menu action for {mremoteng_item.key}",
                )
                menu_action.setProperty("mRemoteNgTopMenuKey", mremoteng_item.key)
                menu_action.setProperty("mRemoteNgTopMenuLabel", mremoteng_item.label)
                menu_action.setProperty(
                    "mRemoteNgTopMenuPrimaryAction",
                    mremoteng_item.primary_action,
                )
                menu_action.setToolTip(mremoteng_item.tooltip)
                self.mremoteng_top_menus.append(menu)
                self.mremoteng_top_menu_actions.append(menu_action)
                if mremoteng_item.key not in self.product_menu_callbacks["mremoteng"]:
                    raise RuntimeError(
                        f"missing mRemoteNG top-menu handler: {mremoteng_item.key}"
                    )
                action = _required_gui_value(
                    menu.addAction(mremoteng_item.primary_action),
                    f"mRemoteNG primary menu action for {mremoteng_item.key}",
                )
                action.triggered.connect(
                    lambda _checked=False, action_key=mremoteng_item.key: self.run_product_menu_action(
                        "mremoteng", action_key
                    )
                )
                action.setProperty("menuActionKey", mremoteng_item.key)
                action.setProperty("menuActionFamily", "mremoteng")
                action.setProperty(
                    "menuActionOperation",
                    self.product_menu_operations["mremoteng"][mremoteng_item.key],
                )
        ...
        def populate_view_design_menu(self) -> None:
            design_menu = _required_gui_value(
                self.view_menu.addMenu("Design preset"),
                "design preset menu",
            )
            for preset in GUI_DESIGN_PRESETS:
        ...
        def add_toolbar_widget(self, toolbar: QToolBar, widget: QWidget):
            action = _required_gui_value(
                ...
                f"toolbar action for {widget.objectName() or type(widget).__name__}",
            )
        ...
        def build_moba_ribbon_buttons(self) -> list[QToolButton]:
            slots: dict[str, Callable[..., object]] = {
        ...
        def create_moba_rail(self) -> QWidget:
            ...
            rail.customContextMenuRequested.connect(
                lambda position, source=rail: self.show_moba_rail_context_menu(
                    source,
                    position,
                )
            )
            ...
            for item in gui_design_moba_rail_items():
                ...
                button.customContextMenuRequested.connect(
                    lambda position, source=button: self.show_moba_rail_context_menu(
                        source,
                        position,
                    )
                )
                ...
                if item.label:
                    label = MobaRailLabel(item.label, item.role, button)
                    label.setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
                    label.customContextMenuRequested.connect(
                        lambda position, source=label: self.show_moba_rail_context_menu(
                            source,
                            position,
                        )
                    )
        ...
        def build_moba_rail_context_menu(self) -> QMenu:
            menu = QMenu(self)
            menu.setObjectName("mobaRailContextMenu")
            menu.addAction("Sessions", self.show_moba_sessions_rail)
            menu.addAction("Favorites", self.show_moba_favorites_rail)
            menu.addSeparator()
            menu.addAction("Tools", self.show_moba_tools_status)
            menu.addAction("Macros", self.show_moba_macros_status)
            sftp_action = _required_gui_value(
                menu.addAction("SFTP browser", self.show_moba_sftp_rail),
                "Moba rail SFTP action",
            )
            sftp_action.setEnabled(self.moba_connected_dock is not None)
            menu.addSeparator()
            menu.addAction("Collapse or expand sessions", self.toggle_moba_session_panel)
            return menu

        def show_moba_rail_context_menu(self, source: QWidget, position: QPoint) -> None:
            menu = self.build_moba_rail_context_menu()
            menu.exec(source.mapToGlobal(position))
            menu.deleteLater()
        ...
        def show_moba_profile_tree(self, *, preserve_connected_dock: bool = False) -> None:
            if not hasattr(self, "moba_left_stack"):
                return
            if self.moba_connected_dock is not None and not preserve_connected_dock:
                dock = self.moba_connected_dock
                self.moba_connected_dock = None
                if hasattr(dock, "shutdown_runtime"):
        ...
        def configure_toolbar_copy_for_design(self, preset: GuiDesignPreset) -> None:
            ...
            for key, label, tooltip in actions:
                ...
                active_remmina_route = (
                    remmina_transfer_route
                    if remmina_transfer_route is not None
                    and key == remmina_transfer_route.toolbar_action_key
                    else None
                )
                is_remmina_transfer = active_remmina_route is not None
                button.setProperty(
                    "remminaSftpTransferRouteKey",
                    active_remmina_route.key if active_remmina_route is not None else "",
                )
                button.setProperty(
                    "remminaSftpTransferRouteRole",
                    active_remmina_route.route_role
                    if active_remmina_route is not None
                    else "",
                )
                button.setProperty(
                    "remminaSftpTransferRouteToolbarActionKey",
                    active_remmina_route.toolbar_action_key
                    if active_remmina_route is not None
                    else "",
                )
                button.setProperty(
                    "remminaSftpTransferRouteToolbarActionLabel",
                    active_remmina_route.toolbar_action_label
                    if active_remmina_route is not None
                    else "",
                )
                button.setProperty(
                    "remminaSftpTransferRouteToolbarActionObject",
                    active_remmina_route.toolbar_action_object
                    if active_remmina_route is not None
                    else "",
                )
                button.setProperty(
                    "remminaSftpTransferRouteSelectedProfileKey",
                    active_remmina_route.selected_profile_key
                    if active_remmina_route is not None
                    else "",
                )
                button.setProperty(
                    "remminaSftpTransferRouteSelectedProfile",
                    active_remmina_route.selected_profile_name
                    if active_remmina_route is not None
                    else "",
                )
                button.setProperty(
                    "remminaSftpTransferRouteProtocol",
                    active_remmina_route.selected_profile_protocol
                    if active_remmina_route is not None
                    else "",
                )
                button.setProperty(
                    "remminaSftpTransferRouteStatus",
                    active_remmina_route.selected_profile_status
                    if active_remmina_route is not None
                    else "",
                )
                button.setProperty(
                    "remminaSftpTransferRouteActiveTab",
                    active_remmina_route.active_tab_label
                    if active_remmina_route is not None
                    else "",
                )
                button.setProperty(
                    "remminaSftpTransferRoutePath",
                    active_remmina_route.remote_path
                    if active_remmina_route is not None
                    else "",
                )
                button.setProperty(
                    "remminaSftpTransferRouteQueueState",
                    active_remmina_route.transfer_status
                    if active_remmina_route is not None
                    else "",
                )
                button.setProperty(
                    "remminaSftpTransferRouteQueueLabel",
                    active_remmina_route.transfer_queue_label
                    if active_remmina_route is not None
                    else "",
                )
                button.setProperty(
                    "remminaSftpTransferRouteRenderSource",
                    active_remmina_route.render_source
                    if active_remmina_route is not None
                    else "",
                )
                if remmina_transfer_route is not None:
        ...
        def focus_interaction_widgets(self) -> dict[str, QLineEdit]:
            def live_line_edit(object_name: str) -> QLineEdit:
        ...
        def set_interaction_state(self, widget, state: str) -> None:
            ...
            widget_style = _widget_style(widget)
        ...
        def show_moba_connected_dock(self, state: MobaConnectedSessionState) -> None:
            if not hasattr(self, "moba_left_stack"):
                return
            if (
                self.moba_connected_dock is not None
                and hasattr(self.moba_connected_dock, "can_reuse_for_state")
                and self.moba_connected_dock.can_reuse_for_state(state)
            ):
        ...
        def apply_moba_top_stack_geometry(self) -> None:
            ...
            self.main_menu_bar.setProperty("mobaTopStackMenuY", stack.menu_y)
            self.main_menu_bar.setProperty(
                "mobaTopStackMenuHeight",
                stack.menu_height,
            )
        ...
        def apply_selected_design(self, *_args) -> None:
            ...
            for catalog_widget in (self, self.design_select, self.main_toolbar):
                ...
                self.apply_preset_isolation_route_properties(
                    catalog_widget,
                    isolation_route,
                )
                self.apply_preset_transition_route_properties(
                    catalog_widget,
                    transition_route,
                )
            self.apply_keyboard_shortcut_route_for_design(keyboard_shortcut_route)
            for reference_widget in (self, self.tabs):
                if reference_tab_route is None:
                    self.clear_preset_reference_tab_route_properties(reference_widget)
                    self.clear_preset_reference_tab_chrome_route_properties(
                        reference_widget
                    )
                    self.clear_preset_reference_status_bar_route_properties(
                        reference_widget
                    )
                    self.clear_preset_reference_session_action_route_properties(
                        reference_widget
                    )
                    self.clear_preset_reference_surface_route_properties(reference_widget)
                    self.clear_preset_reference_control_route_properties(reference_widget)
                    self.clear_preset_reference_input_route_properties(reference_widget)
                    self.clear_preset_reference_transcript_route_properties(
                        reference_widget
                    )
                else:
                    self.apply_preset_reference_tab_route_properties(
                        reference_widget,
                        reference_tab_route,
                    )
                    self.apply_preset_reference_tab_chrome_route_properties(
                        reference_widget,
                        reference_tab_chrome_route,
                    )
                    self.apply_preset_reference_status_bar_route_properties(
                        reference_widget,
                        reference_status_route,
                    )
                    ...
                    self.apply_preset_reference_surface_route_properties(
                        reference_widget,
                        reference_surface_route,
                    )
                    self.apply_preset_reference_control_route_properties(
                        reference_widget,
                        reference_control_route,
                    )
                    self.apply_preset_reference_input_route_properties(
                        reference_widget,
                        reference_input_route,
                    )
                    self.apply_preset_reference_transcript_route_properties(
                        reference_widget,
                        reference_transcript_route,
                    )
            for visual_widget in (
                self,
                self.main_toolbar,
                self.layout_toolbar,
                self.left_panel,
                ...
                self.apply_preset_visual_signature_properties(
                    visual_widget,
                    visual_signature,
                )
            ...
            for selection_widget in (
                ...
                self.apply_preset_selection_route_properties(
                    selection_widget,
                    selection_route,
                )
            ...
            for status_widget in (
                self.statusBar(),
                self.status_notice_label,
                *self.status_segment_labels,
            ):
                if reference_status_route is None:
                    self.clear_preset_reference_status_bar_route_properties(status_widget)
                else:
                    self.apply_preset_reference_status_bar_route_properties(
                        status_widget,
                        reference_status_route,
        ...
        def show_moba_sessions_rail(self, *_args) -> None:
            self.show_moba_profile_tree(preserve_connected_dock=True)
            self.filter_profile_tree_names(None)
            self.profile_list.expandAll()
        ...
        def show_moba_favorites_rail(self, *_args) -> None:
            ...
            self.filter_profile_tree_names(set(favorites))
            if favorites:
        ...
        def show_moba_sftp_rail(self, *_args) -> None:
            self.set_moba_rail_active("sftp")
            if getattr(self, "moba_connected_dock", None) is None:
                state = self.moba_connected_state_in_widget(self.tabs.currentWidget())
            ...
            if getattr(self, "moba_connected_dock", None) is not None:
                ...
                return
            profile = self.selected_profile_for_workflow()
            if profile is not None and profile.protocol.lower() in {"ssh", "sftp"}:
                self.open_files_selected()
                return
            self.statusBar().showMessage("SFTP rail: select or open an SSH/SFTP profile")
            self.show_workflow_dialog(
                "SFTP browser",
                "Browse files through a selected SSH or SFTP profile.",
                [
                    (
                        "Connected session",
                        "not selected",
                        "Open an SSH session to attach its synchronized browser dock.",
                    ),
                    (
                        "Saved profile",
                        profile.name if profile is not None else "none selected",
                        "Select an SSH/SFTP profile in Sessions, then choose SFTP.",
                    ),
                ],
                "The SFTP rail no longer routes to unrelated package controls.",
                actions=[
                    ("Sessions", self.show_moba_sessions_rail),
                    ("New profile", self.create_profile),
                ],
        ...
        def apply_session_action_menu_capture(
            self,
            menu,
            route,
            tab_title: str,
        ...
        def build_tab_context_menu(self, index: int):
            ...
            context_actions: list[QAction] = []

            def add_context_action(key: str, callback) -> None:
                spec = specs[key]
                action = _required_gui_value(
                    menu.addAction(str(spec["label"]), callback),
                    f"tab context action for {key}",
                )
        ...
        def show_tab_context_menu(self, position) -> None:
            index = self.moba_tab_bar.tabAt(position)
            if index < 0:
                return
            if self.tab_role(index) != "new-session":
                self.tabs.setCurrentIndex(index)
            ...
            if menu is not None:
                try:
                    menu.exec(self.moba_tab_bar.mapToGlobal(position))
                finally:
                    menu.deleteLater()
        ...
        def build_profile_context_menu(
            self,
            item: QTreeWidgetItem | None = None,
        ) -> QMenu:
            if item is not None:
                self.profile_list.setCurrentItem(item)
            selected = item or self.profile_list.currentItem()
            profile_name = (
                selected.data(0, Qt.ItemDataRole.UserRole)
                if selected is not None
                else None
            )
            has_profile = isinstance(profile_name, str) and bool(profile_name)
            menu = QMenu(self.profile_list)
            menu.setObjectName("profileTreeContextMenu")
            connect_action = _required_gui_value(
                menu.addAction(
                    "Connect",
                    lambda: self.connect_selected(False),
                ),
                "profile context connect action",
            )
            dry_run_action = _required_gui_value(
                menu.addAction(
                    "Preview command",
                    lambda: self.connect_selected(True),
                ),
                "profile context preview action",
            )
            files_action = _required_gui_value(
                menu.addAction(
                    "Open files / SFTP",
                    self.open_files_selected,
                ),
                "profile context files action",
            )
            for action in (connect_action, dry_run_action, files_action):
                action.setEnabled(has_profile)
            menu.addSeparator()
            edit_action = _required_gui_value(
                menu.addAction("Edit profile", self.edit_selected_profile),
                "profile context edit action",
            )
            remove_action = _required_gui_value(
                menu.addAction("Remove profile", self.remove_selected_profile),
                "profile context remove action",
            )
            edit_action.setEnabled(has_profile)
            remove_action.setEnabled(has_profile)
            menu.addSeparator()
            menu.addAction("New profile", self.create_profile)
            menu.addAction("Refresh profiles", self.refresh_profiles)
            return menu
        ...
        def show_profile_context_menu(self, position: QPoint) -> None:
            ...
            if item is not None:
                self.profile_list.setCurrentItem(item)
            menu = self.build_profile_context_menu(item)
            viewport = _required_gui_value(
                self.profile_list.viewport(),
        ...
        def filter_profile_tree_names(self, allowed_names: set[str] | None) -> None:
            """Show all profile rows, or only an explicit Moba rail name set."""
            ...
            def apply_filter(item) -> bool:
                ...
                own_match = (
                    ...
                    or (isinstance(profile_name, str) and profile_name in allowed_names)
                )
                item_match = own_match if isinstance(profile_name, str) else child_match
                item.setHidden(not item_match)
                if item_match and child_match:
                    item.setExpanded(True)
                return item_match
            ...
            for index in range(self.profile_list.topLevelItemCount()):
                apply_filter(self.profile_list.topLevelItem(index))
            self.profile_list.setProperty(
                "mobaProfileTreeFilterMode",
                "all" if allowed_names is None else "favorites",
            )
            self.profile_list.setProperty(
                "mobaProfileTreeFilterNames",
                [] if allowed_names is None else sorted(allowed_names),
            )
            current = self.profile_list.currentItem()
            if current is not None and not self.profile_tree_item_is_visible(current):
                self.profile_list.setCurrentItem(None)
        ...
        def add_welcome_tab(self, *, select: bool | None = None) -> None:
            ...
            start_button.setIcon(
                _widget_style(self).standardIcon(
                    self.standard_icon("SP_DialogApplyButton")
                )
            )
            ...
            recover_button.setIcon(
                _widget_style(self).standardIcon(
                    self.standard_icon("SP_BrowserReload")
                )
            )
