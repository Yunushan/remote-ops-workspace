...
def test_release_publish_asset_checker_requires_strict_platform_goal_assets_dir() -> None:
    ...
    assert checker.main(["--require-platform-goal-targets", "--tag", "v1.0.25"]) == 2
...
def test_native_manifest_signing_metadata_requires_production_truth(tmp_path: Path) -> None:
    checker = _load_checker()
    manifests = {
        "remote-ops-workspace-v1.0.25-windows-x64-native-manifest.json": [
            {
                "signing": {
        ...
        "remote-ops-workspace-v1.0.25-macos-arm64-native-manifest.json": [
            {
                "signing": {
                    "release_channel": "production-signed",
                    "production_trusted": True,
    ...
    windows_manifest = tmp_path / "remote-ops-workspace-v1.0.25-windows-x64-native-manifest.json"
    ...
    manifests["remote-ops-workspace-v1.0.25-windows-x64-native-manifest.json"][0]["signing"][
        "timestamped"
    ] = False
    (tmp_path / "remote-ops-workspace-v1.0.25-windows-x64-native-manifest.json").write_text(
        json.dumps(manifests["remote-ops-workspace-v1.0.25-windows-x64-native-manifest.json"]),
        encoding="utf-8",
    )
...
def test_expected_release_assets_expand_default_matrix() -> None:
    ...
    assert "remote_ops_workspace-1.0.25-py3-none-any.whl" in assets
    assert "remote-ops-workspace-v1.0.25-windows-x86-setup.exe" in assets
    assert "remote-ops-workspace-v1.0.25-macos-arm64.pkg" in assets
    assert "remote-ops-workspace-v1.0.25-linux-amd64.deb" in assets
    assert "remote-ops-workspace-v1.0.25-linux-aarch64-native-SHA256SUMS.txt" in assets
    assert "remote-ops-workspace-v1.0.25-linux-i386.deb" not in assets
    assert "remote-ops-workspace-v1.0.25-linux-armhf.deb" not in assets
...
def test_expected_release_assets_normalize_to_requested_tag() -> None:
    ...
    assert "remote_ops_workspace-1.0.25-py3-none-any.whl" not in assets
    assert "remote-ops-workspace-v1.0.25-windows-x64-setup.exe" not in assets
...
def test_publish_contract_rejects_gated_default_asset_without_evidence() -> None:
    ...
    linux_job["asset_patterns"].append("remote-ops-workspace-v1.0.25-linux-i386.deb")

    errors = checker.check_publish_contract(matrix, workflow, evidence_registry=_empty_evidence_registry())

    assert any("gated native asset remote-ops-workspace-v1.0.25-linux-i386.deb" in error for error in errors)
...
def test_publish_contract_uses_explicit_empty_platform_registry(monkeypatch) -> None:
    ...
    assert any(
        "default release matrix includes gated native asset remote-ops-workspace-v1.0.25-linux-i386.deb "
        "for linux-i386 without accepted platform evidence for release_tag v1.0.25"
        in error
        for error in errors
    )
...
def test_publish_contract_rejects_xp_asset_without_complete_xp_pair() -> None:
    ...
    windows_job["asset_patterns"].append("remote-ops-workspace-v1.0.25-windows-xp-x86-native.zip")
