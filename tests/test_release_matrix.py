...
def test_release_matrix_requires_script_supported_checksum_asset_pattern() -> None:
    ...
    assert any(
        ...
        and "remote-ops-workspace-v1.0.11-linux-i686-native-SHA256SUMS.txt" in error
        for error in errors
    )
...
def test_release_matrix_rejects_stale_native_asset_pattern_versions() -> None:
    ...
    assert (
        "linux-native asset pattern must use current project version v1.0.11: "
        "remote-ops-workspace-v9.9.9-linux-<amd64|arm64>.deb"
    ) in errors
    assert (
        "linux-i386 asset pattern must use current project version v1.0.11: "
        "remote-ops-workspace-v9.9.9-linux-i386.deb"
    ) in errors
