...
def check_xp_job(workflow: str) -> list[str]:
    ...
    required_snippets = {
        ...
        'python scripts/check_platform_goal_local_evidence.py --root . --release-tag "${{ inputs.release_tag }}" --target "${{ inputs.target }}" --assets-dir "${{ inputs.assets_dir }}" --xp-evidence "${{ inputs.evidence_file }}" --xp-evidence-dir "${{ inputs.evidence_dir }}"': "XP local protected goal evidence preflight",
