...
def test_ci_workflow_rejects_reduced_or_advisory_coverage_gate() -> None:
    checker = _load_checker()
    source = Path(".github/workflows/ci.yml").read_text(encoding="utf-8")
    reduced_threshold = source.replace("--cov-fail-under=77.5", "--cov-fail-under=77.4")
    reduced_validated_total = source.replace("--min-total 77.5", "--min-total 77.4")
    reduced_branch_threshold = source.replace("--min-branches 62.5", "--min-branches 62.4")
