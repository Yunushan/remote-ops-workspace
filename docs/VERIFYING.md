...
- `python scripts/make_extended_linux_evidence_bundle.py --help` for the Linux i386/armhf evidence bundle packer that revalidates candidate evidence against builder identity, native smoke-log proof lines, release asset URLs and artifact SHA-256 values before writing a reviewable manifest, archive containing the hashed artifacts, and SHA-256 sidecar;
- `python scripts/stage_extended_linux_evidence_upload.py --help` for the Linux i386/armhf source-artifact upload stager that copies only expected native artifacts, the finalized evidence record and review-bundle files into `linux-evidence-upload`, preventing raw Linux builder output directories from being uploaded by wildcard and rejecting overlapping source/upload roots;
- `python scripts/check_xp_native_evidence.py --contract` for the Windows XP x86/x64 native evidence JSON contract, including XP x86 SP3, Windows XP Professional x64 Edition SP2, required tracked `scripts/xp_smoke_runner.cmd` per-smoke command provenance, canonical `--proof-file xp-smoke-proof/<smoke_id>.txt` binding, evidence-file `xp smoke target`/`xp smoke release`/`xp smoke id` proof-line binding and SHA-256 checks;
- `python scripts/check_xp_native_evidence_workflow.py` for the dispatch-only `.github/workflows/xp-native-evidence.yml` source-artifact workflow that validates staged XP artifacts/evidence on a self-hosted `xp-evidence` runner and uploads `xp-native-evidence-<target>-<release_tag>` for release import;
...
- `python scripts/make_xp_native_evidence_template.py --help` for the failing-by-default XP evidence bundle template generator used before real XP VM or host evidence is collected; the XP evidence validator rejects leftover `TODO`, `placeholder`, `replace with real` and `template evidence` markers in the JSON or smoke files;
- XP evidence JSON and accepted platform records must use a staged workspace-relative non-hidden artifact validation `--assets-dir`; placeholders such as `<artifact-dir>`, wildcards, absolute paths, workspace-root paths, hidden path segments and reserved repository/control directories are rejected outside documentation examples;
- `python scripts/make_xp_native_evidence_bundle.py --help` for the XP evidence bundle packer that validates XP evidence, the full candidate accepted-evidence record, its XP validation command, release asset URLs and artifacts before writing a reviewable manifest, archive containing the hashed artifacts, and SHA-256 sidecar;
- `python scripts/stage_xp_native_evidence_upload.py --help` for the XP source-artifact upload stager that copies only expected native artifacts, the finalized evidence record and review-bundle files into `xp-evidence-upload`, preventing raw operator-supplied artifact or evidence directories from being uploaded by wildcard and rejecting overlapping artifact/evidence/upload roots;
- `python scripts/check_platform_verified_evidence.py` for accepted evidence registry records in `configs/platform_verified_evidence.json`, which are the only records that can promote Linux i386, Linux armhf or Windows XP native-host readiness and must include release asset URLs under the same release tag, release asset URL binding in the review-bundle manifest, review bundle release asset URLs under the same GitHub repository and release tag, a release-importable GitHub Actions artifact source containing the accepted native and review-bundle files, per-artifact SHA-256 digests, promotion config SHA-256 binding, staged workspace-relative artifact validation paths, Linux builder identity evidence plus its SHA-256, sanitized target-scoped host identity, concrete `rpm`/`rpmbuild` tool paths and `sudo_non_interactive=true`, Linux security patch evidence, Linux native build/smoke command provenance, Linux native smoke evidence SHA-256 and workflow dispatch input binding when applicable, Linux review-bundle smoke logs with command/release/architecture/install/verify/upgrade/uninstall/final-pass proof lines, XP evidence bundle SHA-256, XP evidence validation command binding with staged workspace-relative non-hidden paths, XP evidence contract SHA-256, XP evidence summary, tracked `scripts/xp_smoke_runner.cmd` XP per-smoke command provenance, canonical XP smoke proof-file command binding, canonical XP smoke evidence-file summary binding, XP security patch evidence binding and XP security smoke proof-line binding when applicable, and review bundle manifest/archive/SHA-256 sidecar digests. The default registry check is finalized-only; `--allow-unfinalized-candidates` is only for local candidate validation before append;
- `python scripts/check_protected_platform_goal.py --release-tag v<project.version>` for the four-target protected goal parity report. It reports the exact accepted/required count for linux-i386, linux-armhf, windows-xp-native-x86 and windows-xp-native-x64, and `--require-complete` is the hard gate that must fail until all four targets have accepted evidence for that release. Mixed-release accepted records remain aggregate evidence only and cannot complete the protected goal parity block;
- `python scripts/check_platform_verified_evidence.py --require-goal-targets --release-tag v<project.version>` for the strict XP-native/Linux-i386/Linux-armhf parity gate; this is expected to fail until all four protected accepted evidence records are present for that exact release tag and finalized with review-bundle digests;
- `python scripts/verify.py --quick --no-cli-smoke --require-platform-goal-targets --release-tag v<project.version> --platform-review-bundle-dir <bundle-dir>` for the full strict platform-goal promotion path, including the release publish asset contract with `--require-platform-goal-targets`, tag-scoped accepted evidence checks and downloaded review-bundle artifact validation. Strict platform-goal mode refuses to run without both `--release-tag` and `--platform-review-bundle-dir`;
- `python scripts/make_platform_verified_evidence_record.py --help` for the supported generator that turns validated artifact/evidence inputs into candidate accepted registry records with artifact digests;
- `python scripts/finalize_platform_verified_evidence_record.py --help` for attaching packaged review-bundle manifest, archive, SHA-256 sidecar digests and derived review bundle release asset URLs to a candidate accepted-evidence record before strict promotion or registry append; finalization also rejects review bundles whose manifest release asset URLs or XP validation command drift from the candidate record, and registry append rejects unfinalized platform candidates;
- `python scripts/check_platform_review_bundle_artifacts.py --bundle-dir <bundle-dir> --require-goal-targets --release-tag v<project.version>` after downloading review-bundle artifacts to re-read each bundle ZIP, extract its candidate record, rerun finalization and prove the finalized record matches the accepted registry entry for that exact release tag;
- `python scripts/import_platform_evidence_artifacts.py --release-tag v<project.version> --require-goal-targets --out-dir <release-assets>` in the tagged release workflow to download the accepted evidence records' release-importable GitHub Actions artifacts, require the exact expected root file set without extra files or nested raw builder directories, and copy the exact SHA-bound native and review-bundle files into the publish asset directory;
...
- `python scripts/check_release_publish_assets.py` for publish-time release asset completeness, checksum sidecar and release-manifest contract wiring, including a guard that revalidates finalized platform accepted evidence, requires same-tag accepted evidence when `--require-platform-goal-targets` is used, extends the expected release asset set with accepted protected-platform artifacts and review-bundle files for that tag, requires checksum sidecars to collectively cover every expected non-sidecar file, compares downloaded gated artifact and review-bundle SHA-256 values against accepted records, and re-reads same-tag review-bundle archives from the release asset directory to prove they re-finalize to the accepted registry entries before rejecting Linux i386/armhf and Windows XP native assets unless the required finalized accepted evidence records exist, plus MobaXterm parity accepted-evidence registry validation and the optional `--require-mobaxterm-parity-complete` hard gate for true product-depth parity claims;
